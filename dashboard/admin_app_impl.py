...
wrapper, administrator profile/personalised-invitation routes, public
local-service visibility, and the narrow TG9990 spoken-telemetry control.
"""
...
class BackupRequiredError(RuntimeError):
    """Raised when an admin config mutation has not passed the backup gate."""
...
class AdminIdentityMiddleware:
    ...
    async def _serve_admin_page(self, scope, receive, send):
        ...
        scripts = (
            ...
            f'\n<script src="/static/admin_parrot_voice.js?v={version}"></script>'
            f'\n<script src="/static/local_services.js?v={version}"></script>\n'
        )
    ...
    def _admin_session(self, scope, require_csrf=False):
    ...
    def _cognito_session(self, scope, require_csrf=False):
        session = self._admin_session(scope, require_csrf=require_csrf)
    ...
    @staticmethod
    def _require_backup(session) -> None:
        if not (
            getattr(session, "config_backup_confirmed", False)
            and getattr(session, "config_backup_revision", None)
        ):
            raise BackupRequiredError(
                "Download the current HBlink4 config backup before changing parrot voice telemetry"
            )
    ...
    async def _parrot_voice(self, scope, receive, send, update=False):
        try:
            ...
            store = ParrotVoiceConfigStore(server._talkgroup_store())

            if update:
                payload = await auth_app._read_json(receive)
                if not isinstance(payload, dict):
                    raise ParrotVoiceConfigError("Request body must be a JSON object")
                configuration = store.save_enabled(
                ...
                response = JSONResponse(configuration)
            ...
            response = JSONResponse({"detail": str(exc)}, status_code=428)
        except StaleParrotVoiceConfigError as exc:
            response = JSONResponse({"detail": str(exc)}, status_code=409)
        except ParrotVoiceConfigError as exc:
            response = JSONResponse(
    ...
    async def _profile(self, scope, receive, send, update=False):
        try:
            _session, access_token = self._cognito_session(
                scope, require_csrf=update
            )
            ...
            if update:
                ...
                profile = await asyncio.to_thread(
                    service.get_profile, access_token
                )
            response = JSONResponse(
                {
                    "ok": True,
    ...
    async def _invite(self, scope, receive, send):
        try:
            ...
            status = (
                409
                if "Complete your administrator profile" in str(exc)
                else 400
            )
    ...
    async def _resend(self, scope, receive, send, username):
        try:
            ...
            await asyncio.to_thread(
                service.resend_invite, username, inviter
            )
    ...
    async def __call__(self, scope, receive, send):
        ...
        if method == "GET" and path == "/api/local-services":
            await self._send(
                JSONResponse({"services": _public_local_services()}),
                scope,
                receive,
                send,
            )
        ...
        # This control deliberately works for both Cognito administrators and
        # the permanent local PBKDF2 break-glass administrator.
        if method == "GET" and path == "/api/admin/parrot-voice":
            await self._parrot_voice(scope, receive, send, update=False)
            return
        if method == "PUT" and path == "/api/admin/parrot-voice":
            await self._parrot_voice(scope, receive, send, update=True)
            return

        if (
            server._admin_auth_provider() != "cognito"
        ...
        if (
            method == "POST"
            and path.startswith("/api/admin/users/")
            and path.endswith("/resend-invite")
        ):
            encoded = path[
                len("/api/admin/users/") : -len("/resend-invite")
            ]
